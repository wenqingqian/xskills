...
| x-code-clean | active | Explicit-only: invoked ONLY when the user explicitly requests this skill by name or its keywords (e.g. "x-code-clean", "clean up comments", "trim comments", "dead code", "unused code"); never auto-triggered. Three check categories over a natural-language scope ("this commit", "the whole repo", default uncommitted changes): comment cleanup fanned out to parallel read-only subagents (5–10 text files each; four tiers over comments, docstrings, whitelisted descriptive strings, and doc prose which is report-only; deletes feedback-driven why-not residue, date stamps, skill/session citations, unnecessary cross-file/repo references), style checks (imports not at module top level, with exemption flags for legitimate inner imports), dead-code detection (module-level defs never referenced in the repo, Python only); findings verified then reported, edits only after user confirmation | `x-code-clean [scope in natural language]`; no flags — scope defaults to uncommitted changes |
