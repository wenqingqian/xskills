...
│   │   └── SKILL.md
│   ├── x-code-clean/     # active — comment/text cleanup (dual extraction + subagent fan-out, secrets included) + style + dead-code checkers (explicit invocation only)
│   │   ├── SKILL.md
│   │   ├── GUIDE.md
│   │   └── scripts/
│   │       ├── changed_lines.py
...
| --- | --- | --- |
| v0.11.0 | 2026-09-08 | x-code-clean restatement delete taxonomy (②): code narration, signature restatement, level duplication (one canonical location per mechanism — definition site by default), assert/raise preambles, experiment souvenirs (keep the generalized failure mechanism, never the run's values), process/history narration (prototype/future/review/plan/iteration/exp), TOC docstrings, maintenance imperatives (real obligations become invariants or automated checks), dead/background cross-refs; tier ③ compress targets (public docstrings to purpose + non-obvious semantics, design notes to constraint + consequence, one macro example, provenance with real sync obligations only) |
| v0.10.1 | 2026-09-08 | x-code-clean fan-out concurrency cap: at most 5 subagents run at once (was uncapped); extra partitions queue and spawn as running ones finish — prevents unbounded subagent creation on large scopes |
| v0.10.0 | 2026-09-08 | x-code-clean secrets category (dual pass: new no-secrets checker with high-precision patterns over every non-binary text file + subagent sweep for unstructured secrets; comment/doc-line hits delete the whole line, code hits report-only, loopback/doc-range exemptions flagged not hidden, report must note git history is not scrubbed and live credentials need rotation); comment review returns to dual hard/soft — extract_comments.py restored as the coverage floor with a --lines-json flag consuming changed_lines.py output, subagent fan-out kept as the judgment half; checks.py widened from .py-only to all non-binary text files (PYTHON_ONLY per checker); assert-vs-example rule rewritten as the two-condition keep rule (pitfall note with hazard intrinsic to this code, or macro example above one experiment's scope; experiment results/data delete; assert-first unchanged); t02/t08/t09 restored + t10/t11 added (11 scenarios) |
